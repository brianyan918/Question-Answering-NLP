...
article_title = text.split()[0]
print("Article tile is "+article_title)

entity_to_pronoun = {'PERSON':'Who', 'NORP':'Who', 'FAC':'Where', 'ORG':'Which organization',
...
for token in doc:
    if token.pos_ == 'VERB':
        ...
        statements = textacy.extract.semistructured_statements(doc, article_title, cue=token.lemma_, ignore_entity_case=True)
    ...
    elif token.text in entity_text_list:
        print('FOUND '+token.text)
        for entity in doc.ents:
            if token.text == entity.text:
                print('LABEL IS '+entity.label_)
                entity_question = entity_to_pronoun[entity.label_]
                break
